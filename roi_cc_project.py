...
class CvROI(data.Dataset):
    ...
    def __init__(self, roi_files, img_folder, img_prefix='', img_suffix='', img_channel=None, img_list='', train=False,
        ...
        #     t0 = time.time()
        #     self.preprocessing(self.rois)
        #     print(('...preprocessing       = %s, DONE in %.1f seconds' % (type(preprocessing).__name__, time.time() - t0)))
    ...
    def __getitem__(self, idx):
        ...
        if self.preprocessing is not None:
            sample = torch.from_numpy(np.transpose(self.preprocessing(torch.from_numpy(sample.astype(np.float32))).numpy(), (2, 0, 1))).float()
        else:
...
class UnlabeledImageROI(data.Dataset):
    ...
    def __init__(self, img_path, ROI_size, img_channel=None, preprocessing=None, verbose=True):
        """
        Args:
